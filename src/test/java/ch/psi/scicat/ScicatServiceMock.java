...
public class ScicatServiceMock implements ScicatService {
    ...
    public RestResponse<PublishedData> getPublishedDataById(String doi) {
        Optional<PublishedData> publishedData = publishedDataCollection.stream()
                .filter(p -> p.getDoi().equals(doi))
                .findFirst();
    ...
    @Override
    public RestResponse<PublishedData> createPublishedData(String accessToken, CreatePublishedDataDto dto) {
        if (!isAuthenticated) {
