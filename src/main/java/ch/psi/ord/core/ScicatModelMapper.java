        ...
        .addMappings(
            m -> {
              m.using(scientficMetadaConverter)
                  .map(Dataset::getScientificMetadata, CreateDatasetDto::setScientificMetadata);
              m.using(personListToOwnerString)
                  .map(Dataset::getCreator, CreateDatasetDto::setPrincipalInvestigator);
              m.map(src -> "rocrate", CreateDatasetDto::setOwnerGroup);
              m.using(personListToOwnerString).map(Dataset::getOwner, CreateDatasetDto::setOwner);
              m.map(Dataset::getName, CreateDatasetDto::setDatasetName);
              ...
              m.using(personListToOwnerEmails)
                  .map(Dataset::getOwner, CreateDatasetDto::setContactEmail);
              m.when(isNotNull())
              ...
              m.map(Dataset::getDescription, CreateDatasetDto::setDescription);
