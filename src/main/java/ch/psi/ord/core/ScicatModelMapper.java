...
public class ScicatModelMapper {
    Converter<List<Person>, List<String>> personToStringList = new Converter<>() {
        @Override
    ...
    Converter<List<String>, List<Person>> stringListToPerson = new Converter<>() {
        @Override
    ...
    public ModelMapper createPublicationModelMapper() {
        ...
        mapper.typeMap(Publication.class, CreatePublishedDataDto.class)
                .addMappings(m -> {
                    ...
                    m.when(isNotNull()).using(personToStringList).map(Publication::getCreator,
                            CreatePublishedDataDto::setCreator);
                });

        mapper.typeMap(CreatePublishedDataDto.class, Publication.class)
                .addMappings(m -> {
                    m.map(CreatePublishedDataDto::getDoi, Publication::setIdentifier);
                    m.map(CreatePublishedDataDto::getTitle, Publication::setTitle);
                    m.using(stringListToPerson).map(CreatePublishedDataDto::getCreator,
                            Publication::setCreator);
                });
